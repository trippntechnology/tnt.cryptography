...
/// <summary>
/// Represents a cipher key used by <see cref="CipherAttributes"/>
/// </summary>
public class CipherKey(string encodedValue = "")
{
    /// <summary>
    /// Base64 encoded key
    /// </summary>
    public string EncodedValue { get; set; } = encodedValue;
    ...
    /// </summary>
    public CipherKey() : this(string.Empty)
    {
    }
    ...
    public CipherKey(string password, int saltSize = 0, int iterations = 2, HashAlgorithmName? hashAlgorithmName = null,
      Enumerations.KeySize keySize = Enumerations.KeySize.Bits256) : this(Convert.ToBase64String(CreateKey(password, saltSize, iterations, hashAlgorithmName, keySize)))
    {
    }
    ...
    /// <summary>
    /// Creates a <see cref="CipherKey"/> from a password
    /// </summary>
    /// <param name="password">
    /// Passphrase from which a pseudo-random password will be derived. The derived password will be used 
    /// to generate the encryption key. Passphrase can be any string. In this example we assume that this 
    ...
    /// shorter keys. (Default: Bits256)
    /// </param>
    /// <returns>A new <see cref="CipherKey"/> instance</returns>
    public static CipherKey Create(string password, int saltSize = 0, int iterations = 2, HashAlgorithmName? hashAlgorithmName = null,
    Enumerations.KeySize keySize = Enumerations.KeySize.Bits256)
    {
        var bytes = CreateKey(password, saltSize, iterations, hashAlgorithmName, keySize);
        return new CipherKey(Convert.ToBase64String(bytes));
    }
}